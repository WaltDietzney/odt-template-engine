...
- Priority: Later
- Status: Product direction / not implemented

An application may eventually need to recognize an ODT generated by this
engine, inspect its known named sections and bookmarks, reconstruct structured
CV data, and render that data into another template. A future design may use
stable application/generator metadata, a schema or format version, and
optionally a document identifier. Native structure remains the source of
truth for extraction; no public import API or parser is defined here.

Origin identification is distinct from integrity verification. A metadata
marker can identify document origin, while a full-file hash is not a reliable
default integrity mechanism because normal LibreOffice edits change the file.
Any future signature or integrity contract therefore needs independent
semantics rather than being inferred from identification metadata.
